      ...
      <div className="bg-white rounded-2xl shadow-2xl w-[420px] p-7 flex flex-col gap-5">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Creating inside <span className="font-semibold text-green-700">{dbName}</span>
            </p>
          </div>
        ...
        <div>
          ...
          <input
            ...
            className={`w-full border rounded-xl px-4 py-2.5 text-sm font-mono text-gray-800 outline-none transition-all ${
              error ? "border-red-300 bg-red-50" : "border-gray-200 focus:border-indigo-400 focus:bg-indigo-50/20"
            }`}
          />
          {error
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-green-50 border border-green-100 rounded-xl p-3">
            ...
            <p className="text-[11px] text-green-600 leading-relaxed">If the database already exists, the new collection will be added inside it.</p>
          </div>
          <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-3">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-bold text-indigo-700">Auto-created</span>
            </div>
            <p className="text-[11px] text-indigo-600 leading-relaxed">New databases are provisioned on the fly — no manual setup needed.</p>
          </div>
        </div>

        <div className="flex items-center gap-3 justify-end">
          <button onClick={onClose} className="px-4 py-2 rounded-xl border border-gray-200 text-sm font-medium text-gray-500 hover:text-gray-800 hover:border-gray-300 bg-white transition-all">
            Cancel
          </button>
          <button
            onClick={handleCreate} disabled={loading || !name.trim()}
            className="flex items-center gap-2 px-5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? (
