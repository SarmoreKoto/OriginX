...
export default function CollectionsPanel({
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
      {/* Header */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 px-4 md:px-6 py-4 border-b border-gray-100">
        <div className="flex items-center gap-2.5 flex-wrap">
          ...
          {selectedDb && (
            <>
              <span className="text-gray-300 text-xs hidden sm:inline">in</span>
              <span className="inline-flex items-center gap-1.5 text-[11px] font-bold text-white px-2.5 py-0.5 rounded-full" style={{ background: "#0d3d26" }}>
                ...
                <span className="truncate max-w-[120px] md:max-w-none">{selectedDb}</span>
              </span>
            </>
          )}
        </div>
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          {selectedDb && (
            <button
              ...
              className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-3 md:px-3.5 py-1.5 rounded-xl text-white text-xs font-semibold transition-all shadow-sm active:scale-95"
      ...
      <div className="p-4 md:p-5">
        {!selectedDb ? (
          <EmptyState
            ...
            sub="Click a database to view its collections"
            className="py-16 md:py-20"
          />
        ) : loading ? (
          <SkeletonList count={4} variant="card" />
        ) : collections.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 md:py-14 gap-4">
            ...
            <button
              onClick={onAddCollection}
              className="flex items-center gap-2 px-4 md:px-5 py-2 md:py-2.5 rounded-xl text-white text-xs md:text-sm font-semibold transition-all shadow-sm active:scale-95"
              style={{ background: "#0d3d26" }}
              onMouseEnter={e => (e.currentTarget.style.background = "#0a2e1c")}
              onMouseLeave={e => (e.currentTarget.style.background = "#0d3d26")}
            >
